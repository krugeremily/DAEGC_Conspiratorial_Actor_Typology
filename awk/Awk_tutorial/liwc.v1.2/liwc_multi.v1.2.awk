...
BEGIN {FS="\t";ORS="\t";maxchar=0;minchar=100}
{
    ...
    #NR = Number of records (lines)
        if(NR==FNR)
    { 
         #if $1 field 1 (the tweet text) contains a wildcard at the end ($) we do special substring matching later
        #the / / are around the pattern to be matched, the \escapes the wildcard to search for it as a letter, the $ indicates the end of the line
        if($1~/\*$/){
            ...
            #$1 is the liwc dictionary?, $3 the tweet text?
