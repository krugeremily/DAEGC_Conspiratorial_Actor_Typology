...
parser.add_argument('--embedding_size', default=16, type=int) # needs to be same as in pretrained GAT
...
class DAEGC(nn.Module):
    def __init__(self, num_features, hidden_size, embedding_size, alpha, num_clusters, v=1):
        ...
        # no additional layers added to DAEGC compared to baseline model
...
def trainer(config):
    ...
    for epoch in tqdm(range(args.max_epoch), desc='Training DAEGC'):
        ...
        if epoch % 5 == 0:
            ...
            # Save model state
            torch.save(model.state_dict(), f'../../model/DAEGC_{date}/iter_{iteration}_epoch_{epoch}.pkl')
